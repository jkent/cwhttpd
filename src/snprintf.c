...
/* This is an adaptation from https://github.com/mpaland/printf */

/* (c) Marco Paland (info@paland.com) 2014-2019,
...
typedef void (*out_t)(char c, void *buffer, size_t idx, size_t maxlen);

// internal buffer output
static inline void _out_buffer(char c, void *buffer, size_t idx,
        size_t maxlen)
{
    if (idx < maxlen) {
        ((char *) buffer)[idx] = c;
...
static inline void _out_null(char c, void *buffer, size_t idx, size_t maxlen)
{
    (void) c;
    (void) buffer;
    (void) idx;
...
static size_t _out_rev(out_t out, char *buffer, size_t idx, size_t maxlen,
...
static size_t _ntoa_format(out_t out, char *buffer, size_t idx, size_t maxlen,
        char *buf, size_t len, bool negative, unsigned int base,
        unsigned int prec, unsigned int width, unsigned int flags)
{
    // pad leading zeros
    if (!(flags & FLAGS_LEFT)) {
        if (width && (flags & FLAGS_ZEROPAD) && (negative ||
...
static size_t _ntoa_long(out_t out, char *buffer, size_t idx, size_t maxlen,
...
static size_t _ntoa_long_long(out_t out, char *buffer, size_t idx,
        size_t maxlen, unsigned long long value, bool negative,
...
static size_t _etoa(out_t out, char *buffer, size_t idx, size_t maxlen,
        double value, unsigned int prec, unsigned int width,
        unsigned int flags);
#endif

// internal ftoa for fixed decimal floating point
static size_t _ftoa(out_t out, char *buffer, size_t idx, size_t maxlen,
        double value, unsigned int prec, unsigned int width,
        unsigned int flags)
{
    ...
    // could be 100s of cs overflowing your buffers == bad
        ...
        unsigned int flags)
{
    ...
    // now approximate log10 from the log2 integer part and an expansion of
    // ln around 1.5
    ...
    if (width > minwidth) {
        // we didn't fall-back so subtract the cs required for the exponent
        fwidth -= minwidth;
    } else {
        // not enough cs, so go back to default sizing
        fwidth = 0U;
    }
    ...
    if (minwidth) {
        ...
        idx = _ntoa_long(out, buffer, idx, maxlen, (expval < 0) ? -expval :
                expval, expval < 0, 10, 0, minwidth - 1,
...
// internal vsnprintf
static int _vsnprintf(out_t out, char *buffer, const size_t maxlen,
        const char *format, va_list va)
{
    ...
    while (*format) {
            ...
            if ((*format == 'i') || (*format == 'd')) {
                ...
                if (flags & FLAGS_LONG_LONG) {
                    ...
                    idx = _ntoa_long_long(out, buffer, idx, maxlen,
                            ...
                            0 - value), value < 0, base, precision, width,
                            flags);
#endif
                ...
                if (flags & FLAGS_LONG_LONG) {
                    ...
                    idx = _ntoa_long(out, buffer, idx, maxlen,
                            va_arg(va, unsigned long), false, base, precision,
                            width, flags);
                } else {
        ...
        case 's':
        {
            ...
            unsigned int l = _strnlen_s(p, precision ? precision :
                    (size_t) -1);
            // pre padding
            if (flags & FLAGS_PRECISION) {
